    ...
    <div class="ChatRecords">
      ...
      <div class="chatHistory" v-loading="loading">
        <div class="staffList" ref="list" style="margin-top: 0;  height: calc(100vh - 2.4rem);">
          ...
          <div class='staffName' style="margin-top: 0.2rem;" v-for="(item,index ) of requestDataList" :key="index"
            ...
            <div class="userbox_left" v-if="(item.to == isqwuserid)">
              ...
              <div class="pBox boxhover chat_left">
                ...
                <!-- 视频通话 -->
                <p v-if="item.msgtype == 'voiptext'" class="chatContent">
  ...
  methods: {
      ...
      api.getQwTalkData(params).then((data) => {
        if (data.length > 0) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].msgtype == "voice" || data[i].msgtype == "voiptext") {
    ...
    searchUser(teamid, keyword) {
      ...
      api.getQwUser(params).then((data) => {
        ...
        _this.selectOneName(data.userList[0])
    ...
    // 获取员工对应客户的姓名
    getQwCustomer(userid, pageNumber, pageSize, frist) {
      var _this = this
      var params = {
        userid: userid,
    ...
    // 选择同事
    selectSecondName(data, name) {

      // this.isAllselect = data;
    },
    selectThirdName(data, name) {
