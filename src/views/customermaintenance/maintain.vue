          ...
          <el-tabs v-model="activeName" @tab-click="handleClick" style="width: 100%">
            <div class="select-content" style="margin-top: 0.2rem;    margin: 0.2rem 0.2rem;">
              <el-autocomplete class="el-input-inners" v-model="activeValue" :fetch-suggestions="querySearchId"
                placeholder="请输入同事姓名" :trigger-on-focus="false" clearable @select="activeselectId"
              ...
              <template v-if="firstAllName != ''">
                <div class="staffList staffListBottom" ref="first"
                  ...
                  <div :class="isfirstselect == item.customerid ? 'selectname staffName' : 'staffName'"
                    v-for="item of firstAllName" :key="item.customerid" :label="item.customername"
            ...
            <el-tab-pane label="同事" name="second">
              <template v-if="secondAllName != ''">
                <div class="staffList staffListBottom" ref="first"
                  style="margin-top: 0;  height: calc(100vh - 3.6rem);">
                  <div :class="isfirstselect == item.customerid ? 'selectname staffName' : 'staffName'"
                    v-for="item of secondAllName" :key="item.customerid" :label="item.customername"
                    :value="item.customerid" @click="selectSecondName(item)">
                    <div class="userbox">
                      <img :src="item.avatar" alt="" class="avatar" />
                      <div class="pBox">
                        <!-- {{ item.lastmsgtime }} -->
              ...
              <!-- <div class="staffList" style="margin-top: 0;     height: calc(100vh - 3.6rem);">
                <div :class="isAllselect == item.userid ? 'selectname staffName' : 'staffName'"
                  v-for="item of  secondAllName" :key="item.userid" :label="item.username" :value="item.userid"
                  @click="selectSecondName(item.userid, item.username)">
                  <div class="userbox">
            ...
            <el-tab-pane label="群聊" name="third">
              <template v-if="thirdAllName != ''">
                <div class="staffList staffListBottom" ref="first"
                  style="margin-top: 0;  height: calc(100vh - 3.6rem);">
                  <div :class="isfirstselect == item.customerid ? 'selectname staffName' : 'staffName'"
                    v-for="item of thirdAllName" :key="item.customerid" :label="item.customername"
                    :value="item.customerid" @click="selectThirdName(item)">
                    <div class="userbox">
                      <img :src="item.avatar" alt="" class="avatar" />
                      <div class="pBox">
                        <!-- {{ item.lastmsgtime }} -->
              ...
              <!-- <div class="staffList" style="margin-top: 0;     height: calc(100vh - 3.6rem);">
                <div :class="isAllselect == item.userid ? 'selectname staffName' : 'staffName'"
                  v-for="item of  secondAllName" :key="item.userid" :label="item.username" :value="item.userid"
                  @click="selectThirdName(item.userid, item.username)">
                  <div class="userbox">
                    <img src="../../static/images/ceshi.jpg" alt="" class="avatar" />
                    <div class="pBox">
                      <p class='namedata'> <span>{{ item.username }}</span> <span>刚刚</span> </p>
      ...
      <div class="chatHistory" v-loading="loading">
        <div class="staffList" ref="list" style="margin-top: 0;  height: calc(100vh - 2.4rem);">
          ...
          <div class='staffName' style="margin-top: 0.2rem;" v-for="item of requestDataList" :key="item.seq"
            ...
            <!-- 客户 -->
            <div class="userbox_left" v-if="(item.to == isqwuserid)">
              <img :src="selectfirstPhoto" alt="" class="avatar" />

              <div class="pBox boxhover chat_left">
                <div class="timeLeft">{{ item.msgtime }}</div>
                <p v-if="item.msgtype == 'text'" class="chatContent">{{ item.text }}</p>
                ...
                <div v-if="item.msgtype == 'voice'" class="chatContent">
                  <div :class="voiceId == item.seq ? 'chatContent  chatVoice playAudioVoice' : 'chatContent  chatVoice'"
                ...
                <div v-if="item.msgtype == 'chatrecord'" class="chatContent chatContent_right">
                ...
                <p v-if="item.msgtype == 'revoke'" class="chatContent">【撤销 】</p>
            ...
            <!-- 员工 -->
            <div class="userbox_right" v-if="item.to == isfirstselect">
              <img :src="selectStaffPhoto" alt="" class="avatar" />
              ...
              <div class="pBox boxhover chat_right">
                ...
                <div v-if="item.msgtype == 'chatrecord'" class="chatContent chatContent_left">
                ...
                <!-- <p v-if="item.msgtype == 'voiptext'" class="chatContent">【视觉文本】</p> -->
                <p v-if="item.msgtype == 'sphfeed'" class="chatContent">【视频号转发】</p>
                <p v-if="item.msgtype == 'revoke'" class="chatContent">【撤销 】</p>
                <!-- <p v-if="item.msgtype == 'chatrecord'" class="chatContent">【 聊天记录 】</p> -->
              </div>
            </div>
  ...
  methods: {
    firstScroll(e) {
      ...
      if (scrollTop + 1 >= scrollHeight - windowHeight) {
        this.getQwCustomer(this.isAllselect, this.pageNumber, this.pageSize)
      }
    },
    ...
    handleScroll(e) {
      ...
      var scrollHeight = e.target.scrollHeight;
      if (scrollTop <= 0 && this.noChathistory) {
        this.requestData(this.isqwuserid, this.isfirstselect, this.seq)
      }
      // console.log("距顶部" + scrollTop + "可视区高度" + windowHeight + "滚动条总高度" + scrollHeight);
    },

    requestData(user1, user2, seq) {
      var _this = this
      this.loading = true;
      var list = _this.requestDataList
      var params = {
        user1: user1,
        user2: user2,
        seq: seq
      }
      api.getQwTalkData(params).then((data) => {
        if (data.length > 0) {
          ...
          // const rawHtml = '<span>这是插入的模板字符串</span>';
          // this.$refs.list.innerHTML = rawHtml;
          // this.$refs.list.insertAdjacentHTML('afterbegin', rawHtml);
    ...
    getQwCustomer(userid, pageNumber, pageSize, first) {
      ...
      api.getQwCustomerByUser(params).then((data) => {
        if (data.qwCustomerList != []) {
          var qwCustomerList = data.qwCustomerList
          ...
          if (first == '客户') {
            ...
            console.log('同事')
            // _this.secondAllName = _this.secondAllName.concat(qwCustomerList);
            // if (_this.pageNumber == '1' && _this.secondAllName[0]) {
            //   _this.selectSecondName(_this.secondAllName[0])
            // }


          } else if (first == '群聊') {
            console.log('群聊')
            // _this.thirdAllName = _this.thirdAllName.concat(qwCustomerList);
            // if (_this.pageNumber == '1' && _this.thirdAllName[0]) {
            //   _this.selectThirdName(_this.thirdAllName[0])
            // }
