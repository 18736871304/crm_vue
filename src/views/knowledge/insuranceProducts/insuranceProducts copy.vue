<template>
  <div class="scriptLibrary">
    ...
    <!-- 团队话术 -->
    <div class="teamLanguage" v-if="activeName == '01'">
      <div class="productList" v-loading="loading">

        <el-tree :data="options" :props="GroupdefaultProps" accordion node-key="id" :default-checked-keys="[1]" :expand-on-click-node="false" ref="tree" @node-click="handleNodeClick" :highlight-current="true">
          <span class="custom-tree-node" slot-scope="{ node, data }">
            <span class="tree-lable">{{ data.groupname }}</span>
            <!-- <span class="tree-lable">{{ data.label }}</span> -->
      ...
      <div class="teamMain">{{ teamhaushu }}</div>
    </div>
    ...
    <el-dialog title="新建话术" :visible.sync="dialoghuashu" width="30%" :before-close="handleClose" :close-on-click-modal="false" append-to-body>
      <div class="titleBox">
        <div>
          <p>分组</p>
          ...
          <div class="block">
            ...
            <el-cascader v-model="groupValue" :options="options" @change="handleChange" placeholder="请选择分组" clearable></el-cascader>
          </div>
        </div>
        <div>
          <p>标题</p>
          ...
          <div class="select-content">
            <el-dropdown trigger="click" style="width: 100%" placement="bottom" ref="disTeam">
              ...
              <el-dropdown-menu class="asd" slot="dropdown">
                <el-tree @check="checkTeam" :data="teamDataList" ref="tree" show-checkbox node-key="id" :default-checked-keys="[1]" :props="defaultProps"> </el-tree>
      ...
      <div v-for="(item, index) in dataList" :key="index">
        <div class="mainBox">
          ...
          <el-tabs v-model="item.mainName" @tab-click="mainClick">
            ...
            <el-tab-pane label="图片" name="tupian">
              <div class="listMainbox">
                ...
                <div class="imgupload">
                  <el-upload accept="image/*" action="#" ref="imgupload" list-type="picture-card" :auto-upload="false" :limit="1" :on-change="
                      (file) => {
                    ...
                    <div slot="file" slot-scope="{ file }">
                      <img class="el-upload-list__item-thumbnail" :src="item.img.file.refilepath" alt="" />
                      <span class="el-upload-list__item-actions">
                        <span class="el-upload-list__item-preview" @click="handlePictureCardPreview(item, index)">
                          <i class="el-icon-zoom-in"></i>
                        </span>
                        <span v-if="!item.img.disabled" class="el-upload-list__item-delete" @click="handleRemove(file, index)">
                          <i class="el-icon-delete"></i>
                        </span>
                      </span>
                    </div>
            ...
            <el-tab-pane label="图文" name="tuwen">
              <div class="listMainbox">
                <div style="margin-bottom: 0.15rem">
                  <el-input placeholder="请输入内容" @input="imgTextSelect" v-model="item.imgText.text" clearable> </el-input>
                </div>
            ...
            <el-tab-pane label="PDF" name="pdf">
              <div class="listMainbox">
                <div style="margin-bottom: 0.15rem">
                  <div class="pdfupload">
                    <el-upload accept=".pdf" action="#" ref="pdfupload" list-type="picture-card" :auto-upload="false" :limit="1" :on-change="
                        (file) => {
              ...
              <div class="listMainbox">
                <div style="margin-bottom: 0.15rem">
                  <div class="videoupload">
                    <el-upload accept="video/*" action="#" ref="videoupload" list-type="picture-card" :auto-upload="false" :limit="1" :on-change="
                        (file) => {
  ...
  methods: {
    ...
    querGroup() {
      ...
      api.getTalkTempleteGroup(parms).then((data) => {
        ...
        this.options = data.talkTempleteGroup
      })
    },
    ...
    group_sure() {
      ...
      for (var i = 0; i < this.groupOptions.length; i++) {
        if (this.groupOptions[i].value == this.groupName) {
          parms.upgroupid = this.groupOptions[i].value
      ...
      api.addTalkTempleteGroup(parms).then((data) => {
        console.log(data)
        if (data.code == '0') {
          // groupid: "5be3cc4355e74fecb10fabfae9f11a10"
          console.log(parms)
    ...
    uploadfile(file, fileType, index) {
      console.log(file, fileType, index);
      var _this = this;
    ...
    clickDelete(data) {
      // dataList
      console.log(this.dataList,22222222)
      // this.handleRemove(this.dataList[data].img.file, '0') 

      if (this.dataList.length > 1) {
        let copiedArr = [...this.dataList]; // 复制数组
        copiedArr.splice(data, 1); // 从索引1开始，删除1个元素
        console.log(copiedArr); // 输出：[1, 3, 4, 5]
    ...
    imgSaveToUrl(file, index) {
      this.uploadfile(file.raw, "img", index);
    },
    ...
    handleNodeClick(data) {
      console.log(data);
      this.teamhaushu = data.groupname
    },
